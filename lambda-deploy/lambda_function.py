...
AI_RETRY_BACKOFF_SECONDS = float(os.environ.get("AI_RETRY_BACKOFF_SECONDS", "2.0"))
...
def call_ai(log_message):
    ...
    max_attempts = max(1, AI_MAX_ATTEMPTS)
    last_error = None

    for attempt in range(max_attempts):
        try:
            with urllib.request.urlopen(req, timeout=AI_TIMEOUT_SECONDS) as response_handle:
            ...
            last_error = e
            print(f"AI call attempt {attempt + 1}/{max_attempts} failed:", str(e))
            if attempt < max_attempts - 1:
                # The local model (Ollama) and/or ngrok tunnel can be slow on first request.
                # Backoff reduces "first click fails, second click succeeds" behavior.
